...
test("pusher works properly", async ({ browser, context }) => {
  ...
  while (!finishButtonFound) {
    ...
    if (finishButton) {
      ...
      const question1 = await firstPage
        .locator("h1.mantine-Text-root")
        .textContent();
      const question2 = await secondPage
        .locator("h1.mantine-Text-root")
        .textContent();
      expect(question1 === question2).toBe(true);

      const nextButton = firstPage.getByRole("button", {
        name: "Next question",
      });
      await nextButton.click();
  ...
  const finishText1 = await firstPage
    .locator("h1.mantine-Text-root")
    .textContent();
  const finishText2 = await secondPage
    .locator("h1.mantine-Text-root")
    .textContent();
